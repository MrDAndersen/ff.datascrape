...
scrape_nfl <- function(season = NULL, week = NULL,
  ...
  if(all(position == c("QB", "RB", "WR", "TE", "K" , "DEF", "DL", "LB", "DB")))
    position <- NULL
  ...
  nfl_data <- data.table::rbindlist(lapply(nfl_proj$players, function(p){

    player_data <- p[which(names(p) != "stats")]
    player_stats <- p$stats
    names(player_stats) <- nfl_stats[id %in% names(p$stats)]$shortName

    player_table <- data.table::as.data.table(append(player_data, player_stats))
